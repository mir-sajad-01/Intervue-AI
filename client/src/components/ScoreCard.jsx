...
const ScoreCard = ({ label, value, grade, suffix = '', caption = '' }) => (
  <div className="panel relative overflow-hidden p-4">
    ...
    <div className="mt-1 flex items-end gap-2">
      ...
      {suffix ? <span className="pb-1 text-sm font-semibold text-slate-400">{suffix}</span> : null}
      {grade && <span className={`text-xl font-black ${gradeTone(grade)}`}>{grade}</span>}
    </div>
    {caption ? <p className="mt-2 text-xs font-medium uppercase tracking-wide text-slate-400">{caption}</p> : null}
  </div>
);
